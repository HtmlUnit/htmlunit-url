...
public enum ValidationError {
    ...
    DOMAIN_TO_ASCII(
        "Unicode ToASCII records an error or returns the empty string. [UTS46]\n"
                + ", If details about Unicode ToASCII errors are recorded, "
                + "user agents are encouraged to pass those along.",
        true),
    DOMAIN_TO_UNICODE(
    ...
    DOMAIN_INVALID_CODEPOINT(
        "The input's host contains a forbidden domain code point.\n"
                + "\nHosts are percent-decoded before being processed when the URL is special,"
                + " which would result in the following host portion becoming \"exa#mple.org\""
    ...
    MISSING_SCHEME_NON_RELATIVE_URL(
        "The input is missing a scheme, because it does not begin with an ASCII alpha, and either"
                + " no base URL was provided or the base URL cannot be used as a base URL because"
                + " it has an opaque path.\n"
                + "\n"
    ...
    FILE_INVALID_WINDOWS_DRIVE_LETTER(
        "The input is a relative-URL string that starts with a Windows drive letter"
    ...
    private final boolean isFailure_;

    ValidationError(final String description, final boolean isFailure) {
        description_ = Objects.requireNonNull(description);

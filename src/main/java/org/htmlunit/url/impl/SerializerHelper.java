...
public class SerializerHelper {
    ...
    static int compress(final short[] ipPieces) {
        ...
        for (int i = 0; i < ipPieces.length; i++) {
            final short ipPiece = ipPieces[i];
            if (ipPiece == 0) {
                currentNumberOf0++;
            }
     ...
     */
    static void serialize(final UrlImpl url, final boolean excludeFragment, final StringBuilder output) {
        ...
        if (url.host_ != null) {
            ...
            if (url.includeCredentials()) {
                // 2.2.1
                output.append(url.username_);
                // 2.2.2
                if (!url.password_.isEmpty()) {
                    output.append(":");
                    output.append(url.password_);
                }
                // 2.2.3
                output.append("@");
            }
            ...
            if (url.port_ != null) {
                output.append(":");
                output.append(SerializerHelper.serializeInteger(url.port_));
            }
        }
        // 3
        // If url’s host is null, url does not have an opaque path, url’s path’s size is
        // greater than 1,
        // and url’s path[0] is the empty string, then append U+002F (/) followed by
        // U+002E (.) to
        // output.
        else if (!url.hasAnOpaquePath() && url.path_.size() > 1 && "".equals(url.path_.get(0))) {
            output.append("/.");
        }
        // 4
        ...
        if (url.query_ != null) {
            output.append("?");
            output.append(url.query_);
        }
        // 7
        if (!excludeFragment && url.fragment_ != null) {
            output.append("#");
            output.append(url.fragment_);
        }
    }

    static void serializeHost(final Domain host, final StringBuilder output) {
        Objects.requireNonNull(host);
        Objects.requireNonNull(output);
        output.append(host);
    }

    static void serializeHost(final OpaqueHost host, final StringBuilder output) {
        Objects.requireNonNull(host);
        Objects.requireNonNull(output);
        output.append(host);
    }

    static void serializeHost(final EmptyHost host, final StringBuilder output) {
        Objects.requireNonNull(host);
        Objects.requireNonNull(output);
        output.append(host);
    }
     ...
     * 1) Let output be the empty string. 2) Let n be the value of address. 3) For
     * each i in the range 1 to 4, inclusive: 3.1) Prepend n % 256, serialized, to
     * output. 3.2) If i is not 4, then prepend U+002E (.) to output. 3.3) Set n to
     * floor(n / 256). 4) Return output.
     *
     * @param host   the {@link Ipv4Address} host to serialize
     * @param output the output buffer
     */
    static void serializeHost(final Ipv4Address host, final StringBuilder output) {
        Objects.requireNonNull(host);
        Objects.requireNonNull(output);
        final byte[] bytes = InfraHelper.getBytes(host.ip());
     ...
     * 1) Let output be the empty string. 2) Let compress be an index to the first
     * IPv6 piece in the first longest sequences of address’s IPv6 pieces that are
     * 0. In 0:f:0:0:f:f:0:0 it would point to the second 0. 3) If there is no
     * sequence of address’s IPv6 pieces that are 0 that is longer than 1, then set
     * compress to null. 4) Let ignore0 be false. 5) For each pieceIndex in the
     * range 0 to 7, inclusive: 5.1) If ignore0 is true and address[pieceIndex] is
     * 0, then continue. 5.2) Otherwise, if ignore0 is true, set ignore0 to false.
     * 5.3) If compress is pieceIndex, then: 5.3.1) Let separator be "::" if
     * pieceIndex is 0, and U+003A (:) otherwise. 5.3.2) Append separator to output.
     * 5.3.3) Set ignore0 to true and continue. 5.4) Append address[pieceIndex],
     * represented as the shortest possible lowercase hexadecimal number, to output.
     * 5.5) If pieceIndex is not 7, then append U+003A (:) to output. 6) Return
     * output.
     *
     * @param host   the {@link Ipv6Address} host
     * @param output the output buffer
     */
    static void serializeHost(final Ipv6Address host, final StringBuilder output) {
        Objects.requireNonNull(host);
        Objects.requireNonNull(output);
        final short[] address = host.ip();
        // int compress
        final int compress = compress(address);
        boolean ignore0 = false;
        for (int pieceIndex = 0; pieceIndex <= 7; pieceIndex++) {
            final short ipPiece = address[pieceIndex];
    ...
    static String serializeInteger(final int value) {
     ...
     * @param output the output of the serialization
     */
    static void serializeOrigin(final UrlImpl url, final StringBuilder output) {
        Objects.requireNonNull(url);
        Objects.requireNonNull(output);
        switch (url.scheme_) {
            case "blob":
     ...
     * </pre>
     */
    static void serializePath(final UrlImpl url, final StringBuilder output) {
        if (url.hasAnOpaquePath()) {
            if (!url.path_.isEmpty()) {
                output.append(url.path_.get(0));
            }
            return;
        }
        for (final String segment : url.path_) {
            output.append("/");
     ...
     * @param output the output buffer
     */
    static void serializeFormUrlEncoded(final List<UrlSearchParam> tuples,
            final CharsetEncoder encoder, final StringBuilder output) {
        Objects.requireNonNull(tuples);
        Objects.requireNonNull(encoder);
        Objects.requireNonNull(output);
        for (final UrlSearchParam tuple : tuples) {
            final String encodedName = UrlHelper.percentEncodeAfterEncoding(encoder, tuple.name(),
                    CodepointHelper::isInUrlEncodedPercentEncodeSet, true);
            final String encodedValue = UrlHelper.percentEncodeAfterEncoding(encoder, tuple.value(),
                    CodepointHelper::isInUrlEncodedPercentEncodeSet, true);
            if (output.length() > 0) {
                output.append('&');
            }
    ...
    /**
     * The application/x-www-form-urlencoded byte serializer takes a byte sequence
     * input and then runs these steps: Let output be the empty string. For each
     * byte in input, depending on byte: 0x20 (SP) Append U+002B (+) to output. 0x2A
     * (*) 0x2D (-) 0x2E (.) 0x30 (0) to 0x39 (9) 0x41 (A) to 0x5A (Z) 0x5F (_) 0x61
     * (a) to 0x7A (z) Append a code point whose value is byte to output. Otherwise
     * Append byte, percent-encoded, to output. Return output.
     *
     * @param bytes  the bytes to encode
     * @param output the output of the serialization
     */
    static void urlEncodedSerialize(final byte[] bytes, final StringBuilder output) {
        for (final byte abyte : bytes) {
            if (abyte == 0x20) {
                output.append("+");
            }
            else if (abyte == 0x2A || abyte == 0x2D || abyte == 0x2E || InfraHelper.isAsciiDigit(abyte)
                    || InfraHelper.isAsciiUpperAlpha(abyte) || abyte == 0x5F || InfraHelper.isAsciiLowerAlpha(abyte)) {

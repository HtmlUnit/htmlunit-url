 ...
 * <a href="https://url.spec.whatwg.org/#concept-basic-url-parser"></a>WhatWg
 * basic URL parser</a>
 *
 * @author stephane
 */
class UrlParser {
     ...
     * </ul>
     * </li>
     * <li>2) Otherwise, if one of the following is true: <br>
     * c is the EOF code point, U+002F (/), U+003F (?), or U+0023 (#) <br>
     * url is special and c is U+005C (\) <br>
     * then:
     * <ul>
     * <li>2.1) If atSignSeen is true and buffer is the empty string, host-missing
    ...
    private StateReturnType authorityState() {
        ...
        // 2
                ...
                || (url_.isSpecial() && input.codepointIs(CodepointHelper.CP_BACKSLASH))) {
            // 2.1
     ...
     * or base URL base (default null), an optional encoding encoding (default
     * UTF-8), an optional URL url, and an optional state override state override,
     * and then runs these steps: <br>
     * <br>
     * The encoding argument is a legacy concept only relevant for HTML. The url and
     * state override arguments are only for use by various APIs. <br>
     * <br>
     * When the url and state override arguments are not passed, the basic URL
     * parser returns either a new URL or failure. If they are passed, the algorithm
     ...
     * </ul>
     * <li>2) If input contains any ASCII tab or newline, invalid-URL-unit
     * validation error.</li>
     * <li>3) Remove all ASCII tab or newline from input.</li>
     * <li>4) Let state be state override if given, or scheme start state
     * otherwise.</li>
     * <li>5) Set encoding to the result of getting an output encoding from
     * encoding.</li>
     ...
     * <li>9) Keep running the following state machine by switching on state. If
     * after a run pointer points to the EOF code point, go to the next step.
     * Otherwise, increase pointer by 1 and continue with the state machine.</li>
     * </ul>
     */
    public UrlImpl basicParse(final String input, final UrlImpl base,
        ...
        // 5
        this.encoding = encoding == null ? StandardCharsets.UTF_8 : InfraHelper.getOutputEncoding(encoding);
        // 6
    ...
    /**
     * The URL parser takes a string input, with an optional base URL base and an
     * optional encoding encoding override, and then runs these steps: 1) Let url be
     * the result of running the basic URL parser on input with base, and encoding
     * override as provided. 2) If url is failure, return failure. 3) If url’s
     * scheme is not "blob", return url. 4) Set url’s blob URL entry to the result
     * of resolving the blob URL url, if that did not return failure, and null
     * otherwise. 5) Return url.
     */
    ...
    private CharsetEncoder encoder() {
        if (encoder == null) {
            encoder = encoding.equals(StandardCharsets.UTF_8) ? utf8Encoder() : EncodingHelper.getEncoder(encoding);
        }
        return encoder;
    }
    ...
    /**
     * <ul>
     * <li>1) If c is the EOF code point, U+002F (/), U+005C (\), U+003F (?), or
     * U+0023 (#), then decrease pointer by 1 and then:
     * <ul>
     * <li>1.1) If state override is not given and buffer is a Windows drive letter,
     * file-invalid-Windows-drive-letter-host validation error, set state to path
     * state. <br>
     * <br>
     * This is a (platform-independent) Windows drive letter quirk. buffer is not
     * reset here and instead used in the path state.</li>
     ...
     * <ul>
     * <li>1.3.1) Let host be the result of host parsing buffer with url is not
     * special.</li>
    ...
    private StateReturnType fileHostState() {
        // 1
        if (input.codepointIsOneOf(CodepointHelper.CP_EOF, CodepointHelper.CP_SLASH, CodepointHelper.CP_BACKSLASH,
                CodepointHelper.CP_QUESTION_MARK, CodepointHelper.CP_HASH)) {
     ...
     * <li>2.1.2) If the code point substring from pointer to the end of input does
     * not start with a Windows drive letter and base’s path[0] is a normalized
     * Windows drive letter, then append base’s path[0] to url’s path.</li>
     *
     * <br>
    ...
    private StateReturnType fileSlashState() {
        ...
        // 2
            ...
            // 2.1
            if (base != null && "file".equals(base.scheme_)) {
                // 2.1.1
                url_.host_ = base.host_;
                // 2.1.2
                if (!UrlHelper.startsWithWindowsDriveLetter(input) && !base.path_.isEmpty()
                        && UrlHelper.isNormalizedWindowsDriveLetter(base.path_.get(0))) {
                    url_.path_.add(base.path_.get(0));
                }
            }
            // 2.2
            state(State.PATH);
     ...
     * <li>4.3) Otherwise, if c is U+0023 (#), set url’s fragment to the empty
     * string and state to fragment state.</li>
     * <li>4.4) Otherwise, if c is not the EOF code point:
     * <ul>
     * <li>4.4.1) Set url’s query to null.</li>
     * <li>4.4.2) If the code point substring from pointer to the end of input does
     * not start with a Windows drive letter, then shorten url’s path.</li>
     * <li>4.4.3) Otherwise:
     * <ul>
     * <li>4.4.3.1) File-invalid-Windows-drive-letter validation error.</li>
     * <li>4.4.3.2) Set url’s path to « ». <br>
     * This is a (platform-independent) Windows drive letter quirk.</li>
     * </ul>
     * </li>
     * </ul>
     * <li>4.4.4) Set state to path state and decrease pointer by 1.</li></li>
     * </ul>
     * </li>
    ...
    private StateReturnType fileState() {
        ...
        // 4
        else if (base != null && "file".equals(base.scheme_)) {
            // 4.1
            url_.host_ = base.host_;
            url_.path(base);
            url_.query_ = base.query_;
            // 4.2
     ...
     * validation error.</li>
     * <li>1.2) If c is U+0025 (%) and remaining does not start with two ASCII hex
     * digits, invalid-URL-unit validation error.</li>
     * <li>1.3) UTF-8 percent-encode c using the fragment percent-encode set and
     * append the result to url’s fragment.</li>
     * </ul>
     * </li>
     * <li></li>
     * </ul>
    ...
    private StateReturnType fragmentState() {
        ...
        if (input.codepointIsNot(CodepointHelper.CP_EOF)) {
            ...
            // 1.3
            final String utf8PercentEncode = UrlHelper.utf8PercentEncode(utf8Encoder(), input.codepoint(),
    ...
    /**
     * <ul>
     * <li>1) If state override is given and url’s scheme is "file", then decrease
     * pointer by 1 and set state to file host state.</li>
     * <li>2) Otherwise, if c is U+003A (:) and insideBrackets is false, then:
     * <ul>
     * <li>2.1) If buffer is the empty string, host-missing validation error, return
     * failure.</li>
     * <li>2.2) If state override is given and state override is hostname state,
     * then return.</li>
     * <li>2.3) Let host be the result of host parsing buffer with url is not
     * special.</li>
     * <li>2.4) If host is failure, then return failure.</li>
     * <li>2.5) Set url’s host to host, buffer to the empty string, and state to
     * port state.</li>
     * </ul>
     * </li>
     ...
     * <ul>
     * <li>3.1) If url is special and buffer is the empty string, host-missing
     * validation error, return failure.</li>
     * <li>3.2) Otherwise, if state override is given, buffer is the empty string,
     * and either url includes credentials or url’s port is non-null, return.</li>
     * <li>3.3) Let host be the result of host parsing buffer with url is not
     * special.</li>
     * <li>3.4) If host is failure, then return failure.</li>
     * <li>3.5) Set url’s host to host, buffer to the empty string, and state to
     * path start state.</li>
    ...
    private StateReturnType hostState() {
        // 1
        if (stateOverride != null && "file".equals(url_.scheme_)) {
            input.decreasePointerByOne();
            state(State.FILE_HOST);
        }
        // 2
        else if (input.codepointIs(CodepointHelper.CP_COLON) && !insideBracketsFlag_) {
            // 2.1
            if (buffer().length() == 0) {
                throw validationException(ValidationError.HOST_MISSING);
            }
    ...
    protected void logInfo() {
        logger.log(Level.FINER,
                () -> "codePoint: " + input.codepoint() + " - char: "
                        + (input.isEof() ? "EOF" : Integer.toString(input.codepoint())) + " - index: " + input.pointer()
                        + " - state: " + state());
    }
     ...
     * <li>2) Otherwise, if base has an opaque path and c is U+0023 (#), set url’s
     * scheme to base’s scheme, url’s path to base’s path, url’s query to base’s
     * query, url’s fragment to the empty string, and set state to fragment
     * state.</li>
    ...
    private StateReturnType noSchemeState() {
        ...
        // 3
        else if (!"file".equals(base.scheme_)) {
    ...
    /**
     * <ul>
     * <li>1) If c is U+003F (?), then set url’s query to the empty string and state
     * to query state.
     * <li>2) Otherwise, if c is U+0023 (#), then set url’s fragment to the empty
     ...
     * <li>3.2) If c is U+0025 (%) and remaining does not start with two ASCII hex
     * digits, invalid-URL-unit validation error.
     * <li>3.3) If c is not the EOF code point, UTF-8 percent-encode c using the C0
     * control percent-encode set and append the result to url’s path.
     * </ul>
     * </li>
     * </ul>
     *
    ...
    private StateReturnType opaquePathState() {
        ...
        // 3
            ...
            // 3.3
            if (!input.isEof()) {
                final String utf8PercentEncoded = UrlHelper.utf8PercentEncode(utf8Encoder(), input.codepoint(),
     ...
     * <li>1.3) If c is neither U+002F (/) nor U+005C (\), then decrease pointer by
     * 1.</li>
     * </ul>
     * </li>
     * <li>2) Otherwise, if state override is not given and c is U+003F (?), set
     * url’s query to the empty string and state to query state.</li>
     * <li>3) Otherwise, if state override is not given and c is U+0023 (#), set
     * url’s fragment to the empty string and state to fragment state.</li>
     ...
     * </ul>
     * </li>
     * <li>Otherwise, if state override is given and url’s host is null, append the
     * empty string to url’s path.</li>
     * </ul>
     */
    private StateReturnType pathStartState() {
        ...
        else if (stateOverride != null && url_.host_ == null) {
            url_.path_.add("");
        }
        return StateReturnType.CONTINUE;
    }
    ...
    /**
     * <ul>
     * <li>1) If one of the following is true: <br>
     * c is the EOF code point or U+002F (/) <br>
     * <br>
     * url is special and c is U+005C (\) <br>
     * <br>
     * state override is not given and c is U+003F (?) or U+0023 (#) <br>
     * then:
     * <ul>
     ...
     * (\), append the empty string to url’s path. <br>
     * <br>
     * This means that for input /usr/.. the result is / and not a lack of a path.
     * </li>
     * </ul>
     * </li>
     * <li>1.3) Otherwise, if buffer is a single-dot URL path segment and if neither
     * c is U+002F (/), nor url is special and c is U+005C (\), append the empty
     * string to url’s path.</li>
     * <li>1.4) <Otherwise, if buffer is not a single-dot URL path segment, then:
     * <ul>
     * <li>1.4.1) If url’s scheme is "file", url’s path is empty, and buffer is a
     * Windows drive letter, then replace the second code point in buffer with
     * U+003A (:). <br>
     * <br>
     * This is a (platform-independent) Windows drive letter quirk.</li>
     ...
     * /li>
     * <li>1.5) Set buffer to the empty string.</li>
     * <li>1.6) If c is U+003F (?), then set url’s query to the empty string and
     * state to query state.</li>
     * <li>1.7) If c is U+0023 (#), then set url’s fragment to the empty string and
     * state to fragment state.</li>
     * </ul>
     * /li>
     ...
     * validation error.</li>
     * <li>2.2) If c is U+0025 (%) and remaining does not start with two ASCII hex
     * digits, invalid-URL-unit validation error.</li>
     * <li>2.3) UTF-8 percent-encode c using the path percent-encode set and append
     * the result to buffer.</li>
     * </ul>
     * </li>
     * </ul>
     */
    private StateReturnType pathState() {
        ...
        if (input.codepointIsOneOf(CodepointHelper.CP_EOF, CodepointHelper.CP_SLASH)
                || (url_.isSpecial() && input.codepointIs(CodepointHelper.CP_BACKSLASH)) || (stateOverride == null
                        && (input.codepointIsOneOf(CodepointHelper.CP_QUESTION_MARK, CodepointHelper.CP_HASH)))) {
            // 1.1
            if (url_.isSpecial() && input.codepointIs(CodepointHelper.CP_BACKSLASH) /* \ */) {
                validationError(ValidationError.INVALID_REVERSE_SOLIDUS);
            }
            // 1.2
            ...
            // 1.3
            else if (UrlHelper.isSingleDotPathSegment(buffer()) && (!input.codepointIs(CodepointHelper.CP_SLASH)
                    && !(url_.isSpecial() && input.codepointIs(CodepointHelper.CP_BACKSLASH)))) {
                url_.path_.add("");
            }
            // 1.4
     ...
     * c is the EOF code point, U+002F (/), U+003F (?), or U+0023 (#) <br>
     * url is special and c is U+005C (\) <br>
     * state override is given <br>
     * then:
     * <ul>
     * <li>2.1) If buffer is not the empty string, then:
     * <ul>
     * <li>2.1.1) Let port be the mathematical integer value that is represented by
     * buffer in radix-10 using ASCII digits for digits with values 0 through
     * 9.</li>
     * <li>2.1.2) If port is greater than 216 − 1, port-out-of-range validation
     * error, return failure.</li>
     * <li>2.1.3) Set url’s port to null, if port is url’s scheme’s default port;
     * otherwise to port.</li>
    ...
    /**
     * <ul>
     * <li>1) If encoding is not UTF-8 and one of the following is true: <br>
     * url is not special <br>
     * url’s scheme is "ws" or "wss" <br>
     * then set encoding to UTF-8.</li>
     * <li>2) If one of the following is true: <br>
     * state override is not given and c is U+0023 (#) <br>
     * c is the EOF code point <br>
     * then:
     * <ul>
     * <li>2.1) Let queryPercentEncodeSet be the special-query percent-encode set if
     * url is special; otherwise the query percent-encode set.</li>
     * <li>2.2) Percent-encode after encoding, with encoding, buffer, and
     * queryPercentEncodeSet, and append the result to url’s query. <br>
     * <br>
     * This operation cannot be invoked code-point-for-code-point due to the
     * stateful ISO-2022-JP encoder.</li>
     * <li>2.3) Set buffer to the empty string.</li>
     * <li>2.4) If c is U+0023 (#), then set url’s fragment to the empty string and
     * state to fragment state.</li>
     * </ul>
     * </li>
    ...
    private StateReturnType queryState() {
        ...
        if (!StandardCharsets.UTF_8.equals(encoding)
                && (!url_.isSpecial() || ("ws".equalsIgnoreCase(url_.scheme_) || "wss".equalsIgnoreCase(url_.scheme_)))) {
            encoding = StandardCharsets.UTF_8;
        }
        // 2
        if ((stateOverride == null && input.codepointIs(CodepointHelper.CP_HASH)) || input.isEof()) {
            // 2.1, 2.2
            final IntPredicate queryPercentEncodeSet = url_.isSpecial() ? CodepointHelper::isSpecialQueryPercentEncodeSet
                    : CodepointHelper::isQueryPercentEncodeSet;
            // 2.2
            final String percentEncodeAfterEncoding = UrlHelper.percentEncodeAfterEncoding(encoder(), buffer,
     ...
     * <li>3) Otherwise, set url’s username to base’s username, url’s password to
     * base’s password, url’s host to base’s host, url’s port to base’s port, state
     * to path state, and then, decrease pointer by 1.</li>
     * </ul>
     */
    private StateReturnType relativeSlashState() {
        // 1
        if (url_.isSpecial() && (input.codepointIsOneOf(CodepointHelper.CP_SLASH, CodepointHelper.CP_BACKSLASH))) {
            // 1.1
            if (input.codepointIs(CodepointHelper.CP_BACKSLASH)) {
                validationError(ValidationError.INVALID_REVERSE_SOLIDUS);
            }
        ...
        // 3
            ...
            url_.usernamePasswordHostPort(base);
            state(State.PATH);
            input.decreasePointerByOne();
        }
        return StateReturnType.CONTINUE;
     ...
     * invalid-reverse-solidus validation error, set state to relative slash
     * state.</li>
     * <li>5) Otherwise:
     * <ul>
     * <li>5.1) Set url’s username to base’s username, url’s password to base’s
     * password, url’s host to base’s host, url’s port to base’s port, url’s path to
     * a clone of base’s path, and url’s query to base’s query.</li>
     * <li>5.2) If c is U+003F (?), then set url’s query to the empty string, and
     * state to query state.</li>
     * <li>5.3) Otherwise, if c is U+0023 (#), set url’s fragment to the empty
     * string and state to fragment state.</li>
    ...
    private StateReturnType relativeState() {
        ...
        // 4
        else if (url_.isSpecial() && input.codepointIs(CodepointHelper.CP_BACKSLASH)) {
            validationError(ValidationError.INVALID_REVERSE_SOLIDUS);
            state(State.RELATIVE_SLASH);
        }
        // 5
            ...
            // 5.1
            url_.usernamePasswordHostPortPath(base);
            url_.query_ = base.query_;
            // 5.2
            if (input.codepointIs(CodepointHelper.CP_QUESTION_MARK)) {
                url_.query_ = "";
    ...
    /***
     * <ul>
     * <li>1) if c is an ASCII alpha, append c, lowercased, to buffer, and set state
     * to scheme state.
     * <li>2) Otherwise, if state override is not given, set state to no scheme
     ...
     * <li>
     * <ul>
     * <li>2.1.1) If url’s scheme is a special scheme and buffer is not a special
     * scheme, then return.
     * <li>2.1.2) If url’s scheme is not a special scheme and buffer is a special
     * scheme, then return.
     * <li>2.1.3) If url includes credentials or has a non-null port, and buffer is
     * "file", then return.
     * <li>2.1.4) If url’s scheme is "file" and its host is an empty host, then
     * return.
     * </ul>
     * </li>
     ...
     * <li>
     * <ul>
     * <li>2.3.1) If url’s port is url’s scheme’s default port, then set url’s port
     * to null.
     * <li>2.3.2) Return.
     * </ul>
     * </li>
     ...
     * <li>
     * <ul>
     * <li>2.5.1) If remaining does not start with "//",
     * special-scheme-missing-following-solidus validation error.
     * <li>2.5.2) Set state to file state.
     * </ul>
     * </li>
     * <li>2.6) Otherwise, if url is special, base is non-null, and base’s scheme is
     * url’s scheme:
     * <li>
     * <ul>
     * <li>2.6.1) Assert: base is is special (and therefore does not have an opaque
     * path).
     * <li>2.6.2) Set state to special relative or authority state.
     * </ul>
     * </li>
     * <li>2.7) Otherwise, if url is special, set state to special authority slashes
     * state.
     * <li>2.8) Otherwise, if remaining starts with an U+002F (/), set state to path
     * or authority state and increase pointer by 1.
     * <li>2.9) Otherwise, set url’s path to the empty string and set state to
     * opaque path state.
     * </ul>
     * </li>
     * <li>3) Otherwise, if state override is not given, set buffer to the empty
     * string, state to no scheme state, and start over (from the first code point
     * in input).
     ...
     * This indication of failure is used exclusively by the Location object’s
     * protocol setter. <br>
     * Furthermore, the non-failure termination earlier in this state is an
     * intentional difference for <br>
     * defining that setter.
     */
    private StateReturnType schemeState() {
        // 1
        if (InfraHelper.isAsciiAlphanumeric(input.codepoint()) || input.codepointIsOneOf(CodepointHelper.CP_PLUS,
            ...
            // 2.6
            else if (url_.isSpecial() && base != null && base.scheme_.equals(url_.scheme_)) {
    ...
    /**
     * <ul>
     * <li>1) If c is neither U+002F (/) nor U+005C (\), then set state to authority
     * state and decrease pointer by 1.</li>
     * <li>2) Otherwise, special-scheme-missing-following-solidus validation
     * error.</li>
     * </ul>
     */
    ...
    /**
     * <ul>
     * <li>1) If c is U+002F (/) and remaining starts with U+002F (/), then set
     * state to special authority ignore slashes state and increase pointer by
     * 1.</li>
     * <li>2) Otherwise, special-scheme-missing-following-solidus validation error,
     * set state to special authority ignore slashes state and decrease pointer by
     * 1.</li>
     * </ul>
     */
    ...
    /**
     * <ul>
     * <li>1) c is U+002F (/) and remaining starts with U+002F (/), then set state
     * to special authority ignore slashes state and increase pointer by 1.</li>
     * <li>2) Otherwise, special-scheme-missing-following-solidus validation error,
    ...
    protected void stateMachine() {
        logger.log(Level.FINER, () -> "Starting state machine - position: " + input.pointer() + " - base: " + base

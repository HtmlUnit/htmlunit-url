...
public class HostParser {
    ...
    static boolean endsInANumber(final String input) {
        // 1
        final List<String> parts = InfraHelper.strictSplit(input, '.');
        if (parts.isEmpty()) {
            return false;
        }
    ...
    private static boolean hasForbiddenDomainCodepoints(final String domain) {
     ...
     * <ul>
     * <li>1.1) If input does not end with U+005D (]), IPv6-unclosed validation
     ...
     * </ul>
     * </li>
     * <li>2) If isOpaque is true, then return the result of opaque-host parsing
     * input.</li>
     * <li>3) Assert: input is not the empty string.</li>
     * <li>4) Let domain be the result of running UTF-8 decode without BOM on the
     * percent-decoding of input. Alternatively UTF-8 decode without BOM or fail can
     * be used, coupled with an early return for failure, as domain to ASCII fails
     * on U+FFFD (�).</li>
     * <li>5) Let asciiDomain be the result of running domain to ASCII with domain
     * and false.</li>
     * <li>6) If asciiDomain is failure, then return failure.</li>
     * <li>7) If asciiDomain contains a forbidden domain code point,
     * domain-invalid-code-point validation error, return failure.</li>
     * <li>8) If asciiDomain ends in a number, then return the result of IPv4
     * parsing asciiDomain.</li>
     ...
     * @return the Host
     */
    static Host parse(final String input, final boolean isOpaque, final Consumer<ValidationError> errorHandler) {
        // 1
        if (input.startsWith("[")) {
            // 1.1
            if (!input.endsWith("]")) {
        ...
        // 3
        // 4
        final String domain = EncodingHelper.utf8DecodeWithoutBom(UrlHelper.percentDecode(input));
        // 5, 6
        final String asciiDomain = UrlHelper.domainToAscii(domain, false);
        // 7
     ...
     */
    static Ipv4Address parseIpv4(final String input, final Consumer<ValidationError> errorHandler) {
        // 1
        final List<String> parts = InfraHelper.strictSplit(input, '.');
        // 2
        ...
        // 4
        final int[] numbers = new int[parts.size()];
        // 5
        for (int i = 0; i < parts.size(); i++) {
            final String part = parts.get(i);
            try {
                // 5.1
                final Map.Entry<Integer, Boolean> parsedNumber = parseIpv4Number(part);
                // 5.3
     ...
     *
     * @param input the input to parse
     * @return a tuple whose key is the parsed number and the value is a boolean
     *         indicating whether a validation error occurred
     */
    static Map.Entry<Integer, Boolean> parseIpv4Number(String input) {
        ...
        // 8
        final int output = Integer.parseUnsignedInt(input, radix);
        // 9
    ...
    static Ipv6Address parseIpv6(final String input) {
        ...
        if (UrlHelper.codePoint(input, pointer) == 0x003A/* : */) {
            // 5.1
            if (!UrlHelper.remainingMatch(input, pointer, 1, (idx, cp) -> cp == CodepointHelper.CP_COLON)) {
        ...
        while ((UrlHelper.codePoint(input, pointer)) != CodepointHelper.CP_EOF) {
            ...
            if (UrlHelper.codePoint(input, pointer) == CodepointHelper.CP_PERIOD) {
                ...
                while (UrlHelper.codePoint(input, pointer) != CodepointHelper.CP_EOF) {
                    ...
                    // 6.5.5.2
                    if (numberSeen > 0) {
                        if (UrlHelper.codePoint(input, pointer) == CodepointHelper.CP_PERIOD && numberSeen < 4) {
                            // 6.5.5.2.1
                            pointer++;
                        }
                        else {
                    ...
                    while (InfraHelper.isAsciiDigit(UrlHelper.codePoint(input, pointer))) {
                        // 6.5.5.4.1
                        final short number = (short) Character.digit(UrlHelper.codePoint(input, pointer), 10);
                        // 6.5.5.4.2
                        if (ipv4Piece == null) {
                            ipv4Piece = number;
                        }
        ...
        if (compress != null) {
            ...
            while (pieceIndex > 0 && swaps > 0) {
                final int tmp1 = address[pieceIndex];
                final int tmp2 = address[compress + swaps - 1];
     ...
     * </pre>
     */
    static Host parseOpaqueHost(final String input, final Consumer<ValidationError> errorHandler) {
        for (int i = 0; i < input.length(); i++) {
        ...
        // 4
        return OpaqueHost.create(UrlHelper.utf8PercentEncode(input, CodepointHelper::isInC0ControlPercentEncodeSet));
    }
}
